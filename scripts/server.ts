...
let databaseUrl: string = "mongodb+srv://testtest:CoolCool@cluster0.xr9bo.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
let credentialsCollection: Mongo.Collection;
...
async function connectToDB (_url: string): Promise<void> {
    ...
    credentialsCollection = mongoClient.db("Rezeptbuch_Gis").collection("credentials");
    rezeptCollection = mongoClient.db("Rezeptbuch_Gis").collection("Rezepte");
}
...
function handleLogin(req: Http.IncomingMessage, res: Http.ServerResponse): void {
    ...
    req.on("end", async () => {
        ...
        let login: boolean = await isRegistered(credentialsJSON.email, credentialsJSON.password);
        if (login) {
...
async function isRegistered(email: string, password: string): Promise<boolean> {
    let credentialsCursor: Mongo.Cursor<string> = await credentialsCollection.find({"email": email, "password": password});
...
async function handleRegestration(req: Http.IncomingMessage, res: Http.ServerResponse): Promise<void> {
    ...
    req.on("end", async () => {
        let credentialsJSON: Credentials = JSON.parse(data);
        let credentialsCursor: Mongo.Cursor<string> = await credentialsCollection.find({"email": credentialsJSON.email});
        let credentailsArray: string[] = await credentialsCursor.toArray();
        if (credentailsArray.length === 0) {
            credentialsCollection.insertOne({
