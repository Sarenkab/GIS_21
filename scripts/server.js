...
function handleLogin(req, res) {
    ...
    req.on("end", async () => {
        ...
        let login = await isRegistered(credentialsJSON.email, credentialsJSON.password);
        if (login) {
...
async function isRegistered(email, password) {
    let credentialsCursor = await credentialsCollection.find({ "email": email, "password": password });
...
async function handleRegestration(req, res) {
    ...
    req.on("end", async () => {
        let credentialsJSON = JSON.parse(data);
        let credentialsCursor = await credentialsCollection.find({ "email": credentialsJSON.email });
        let credentailsArray = await credentialsCursor.toArray();
