...
async function generateRezept(): Promise<void> {
    ...
    let reponse: Response = await fetch(`https://sarenkahasanewapp.herokuapp.com/rezept?id=${id}`);
    ...
    rezept.zutatenliste.forEach((element: Zutat) => {
    ...
    rezept.schritte.forEach((element: string, index: number) => {
        ...
        schrittNummer.innerText = "Schritt " + (index + 1).toString();
